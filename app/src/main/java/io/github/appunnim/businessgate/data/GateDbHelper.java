...
public final class GateDbHelper extends SQLiteOpenHelper {
    ...
    public GateDbHelper(Context context) {
        super(context, "gate.db", null, 4, db -> {
    ...
    @Override public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 1 || oldVersion >= newVersion || newVersion != 4) throw new IllegalStateException("MIGRATION_UNSUPPORTED");
        ...
        if (oldVersion <= 3) executeAsset(db, "migrations/3-4.sql");
    }
