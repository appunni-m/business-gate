...
- Pure suite: 196,776 assertions; 31 SQLite schema/migration checks; 193 document-qualified acceptance references validated. These counts do not establish 193 completed acceptance cases.
...
- API 36 arm64 emulator performance: 10,000 synthetic records, 20 completed queries, recorded p95 20 ms and database plus WAL 3,866,296 bytes; 50,000-record snapshot loaded in 3,098 ms. The 50,000-record boundary retains a new explicit ALLOW choice by evicting optional cache only. Physical scrolling and phone performance remain unmeasured.
