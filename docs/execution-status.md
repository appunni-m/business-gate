...
The current objective is governed by the [business-name visibility design](name-visibility-design.md). It supersedes the earlier exclusion of notification suppression and name-based permission. Business-name choices and their migration are implemented; notification and entry enforcement are still unavailable. The pre-display integration/platform dependency remains unresolved.
